...
const minifiedSnippet =
  `(()=>{function v({getGlobal:t,setGlobal:d,translateArguments:u,syncItemsCallback:c,usesKeyboardCallback:f,usesPinchZoomCallback:l}){let o={},b=t();d(function(...r){let n=u({originalArguments:r,accessibilityEventParameters:o});b.apply(window,n)});try{a({mediaFeature:"prefers-reduced-motion",abbreviation:"prm",possibleValues:["no-preference","reduce"]}),a({mediaFeature:"prefers-color-scheme",abbreviation:"pcs",possibleValues:["light","dark"]}),a({mediaFeature:"inverted-colors",abbreviation:"ic",possibleValues:["none","inverted"]}),a({mediaFeature:"forced-colors",abbreviation:"fc",possibleValues:["none","active"]}),a({mediaFeature:"prefers-contrast",abbreviation:"pc",possibleValues:["no-preference","more","less","custom"]}),c&&c()}catch(e){console.error(e)}(function(){try{let r=setInterval(function(){let i=document.querySelector(":focus-visible");if(!i)return;let s=i.tagName.toUpperCase();s==="INPUT"||s==="TEXTAREA"||s==="SELECT"||i.contentEditable!=="true"&&(o.uk=!0,clearInterval(r),f&&f())},500)}catch(r){console.error(r)}})(),function(){try{let r=window.visualViewport.scale,n=setInterval(function(){window.visualViewport.scale!==r&&(o.upz=!0,clearInterval(n),l&&l())},500)}catch(r){console.error(r)}}();function a({mediaFeature:e,abbreviation:r,possibleValues:n}){if(m({mediaFeature:e})===!1)return;let s=n.map(p=>({possibleValue:p,mediaQueryResult:window.matchMedia(\`(\${e}: \${p})\`).matches})).find(({_:p,mediaQueryResult:y})=>y===!0);if(s===void 0){console.error(\`Something went wrong. Is there a new \${e} allowed value not accounted for here?\`);return}o[r]=s.possibleValue}function m({mediaFeature:e}){return window.matchMedia(\`not all and (\${e}), (\${e})\`).matches?!0:(console.warn(\`Your browser doesn't support \${e} yet\`),!1)}}function h(){v({getGlobal:()=>window.gtag,setGlobal:t=>{window.gtag=t},translateArguments:({originalArguments:t,accessibilityEventParameters:d})=>{if(t[0]!=="event")return t;let u=t[1],c=Object.fromEntries(Object.entries(d).map(([b,a])=>[\`\${b}__\${u}\`.replaceAll("-","_").replaceAll(" ","_"),a])),l={...t[2],...c},o=[...t];return o[2]=l,o},syncItemsCallback:globalThis.a11y_analytics_config?.providers?.ga?.callbacks?.onSyncItemsResolved,usesKeyboardCallback:globalThis.a11y_analytics_config?.providers?.ga?.callbacks?.onUsesKeyboardResolved,usesPinchZoomCallback:globalThis.a11y_analytics_config?.providers?.ga?.callbacks?.onUsesPinchZoomResolved})}h();})();`;
